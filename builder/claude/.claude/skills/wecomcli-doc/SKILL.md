---
name: wecomcli-doc
description: 企业微信文档、表格（在线表格）、智能表格和智能文档（原名智能主页）管理技能。提供文档的创建、读取、编辑能力，表格和智能表格的内容读取，智能表格的创建，以及智能文档的创建和内容导出。适用场景：(1) 以 Markdown 格式获取文档/表格/智能表格完整内容 (2) 新建文档或智能表格 (3) 用 Markdown 格式覆写文档内容 (4) 创建智能文档，将本地 Markdown 文件发布为智能文档 (5) 导出智能文档内容为 Markdown。支持通过 docid 或文档 URL 定位文档。用户提及企微文档 URL 时触发该技能，两个 host 都属于本技能：`https://doc.weixin.qq.com/xxxx`（文档/表格/智能表格/智能文档），以及智能文档对外分享链接 `https://page.weixin.qq.com/smartpage/*`。
metadata:
  requires:
    bins: ["wecom-cli"]
...
| `https://doc.weixin.qq.com/smartpage/*`<br>`https://page.weixin.qq.com/smartpage/*` | **智能文档**（原名智能主页） | `smartpage_export_task` → `smartpage_get_export_result` |
...
- URL 路径以 `/smartpage/*` 开头 → 智能文档（原名智能主页） → 用 `smartpage_export_task`
  - 智能文档有两种 host：编辑态 `doc.weixin.qq.com/smartpage/*` 与对外分享态 `page.weixin.qq.com/smartpage/p/<docid>?p=xxx`，二者都属本品类，整条 url 可直接传接口。
...
适用 URL：`/smartpage/*`（两种 host 皆可：`doc.weixin.qq.com/smartpage/*` 与对外分享的 `page.weixin.qq.com/smartpage/*`）
...
```
> 对外分享链接形态 `https://page.weixin.qq.com/smartpage/p/<docid>?p=xxx` 同样适用，整条 url 直接传即可。
