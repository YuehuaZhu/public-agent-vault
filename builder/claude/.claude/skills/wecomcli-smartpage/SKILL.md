...
- `smartpage_export_task` 按智能文档 `docid` 导出**整份文档及全部子页面**。
- URL 查询参数 `p=<子页面ID>` 只用于网页端选中当前子页面；当前 CLI 不会据此筛选导出内容，也不能把 `p` 映射为子页面标题。
- 接口接受含 `p` 的完整 URL，不代表返回内容仅属于该子页面。
- **严禁**把导出结果的第一段、页面顺序或上下文猜测为 `p` 指向的当前子页面。
...
1. 用户明确要求整份智能文档或全部子页面时，导出并总结完整内容。
2. 用户明确要求某个子页面时，无论 URL 是否含 `p`，都必须先唯一定位：
   - 用户提供当前页标题，或可从截图/原文中核验出页面标题时，仅按**精确标题**定位目标 Markdown 标题；内容范围从该标题开始，到下一个同级或更高层级标题之前结束。
   - 精确标题出现多次、标题层级不明确或无法唯一定位时，停止总结并请用户提供更明确的页面标题、截图或原文。
   - 用户未提供任何可验证的页面标识时，停止总结并请用户提供当前页标题或截图；URL 含 `p` 时同时说明 CLI 无法解析该参数。
3. 用户未限定子页面且 URL 含 `p` 时，导出结果仍是整份智能文档；不得把 `p` 当作已完成单页定位。
4. 用户未限定子页面且 URL 不含 `p` 时，将导出结果视为整份智能文档，不声称它是某个指定子页面。

不得用标题相似度、导出顺序、第一段内容或业务上下文推断目标子页面。
...
- 总结单个子页面时，必须先按「读取与总结范围」确认能否唯一定位，再输出子页面摘要。
